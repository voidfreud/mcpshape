...
4. **Review** on the branch, before the pull request opens, by a reviewer that did not write
   ...
   - adversarial, when code or a rule file changed: every way the diff can be made to do what a
     ...
     mean to change, each with the inputs or the sequence that does it, the criterion, decision,
     or rule it contradicts, and, for code, the test that would pin it; a finding without that
     sequence is not one and is dropped. A confirmed one, reproduced by the session, is fixed in
     the same wave, with that test for code, or becomes a follow-up when a ticket of the wave, or
     the rule that rule files and code never change in one pull request, rules that out.
   Every finding is fixed on the branch or answered; none is deferred. When any finding was fixed
   rather than answered, a reviewer that did not write the fixes checks, once, that each does
   what its finding asked and breaks nothing else, given the findings and the diff of the fixes;
   its findings are fixed or answered the same way, and there is no third review. The session
   landing the wave reads the riskiest file itself, whoever wrote it.
