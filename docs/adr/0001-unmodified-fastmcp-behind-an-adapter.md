...
# Build on unmodified FastMCP, behind our own Adapter
...
We decided: depend on FastMCP as a normal, unmodified dependency pinned to a major version, and never fork or patch it. Every FastMCP touchpoint goes through the Adapter, one small module, and user-facing surfaces (Hook and Virtual Tool decorators, config files, the management API) never expose FastMCP types. Only the Adapter changes when FastMCP does.
...
- **Use FastMCP directly, no Adapter.** Rejected: user Hook files and our config schema would break on every FastMCP major.
...
- Upgrading a FastMCP major is a deliberate, tested change to the Adapter, not a routine bump.
