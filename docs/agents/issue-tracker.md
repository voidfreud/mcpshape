...
- **Threads**: when a wave's tickets are worked in parallel, each worker starts from the wave's branch and proves it by quoting the commit it is on; the lead copies each thread's commits onto the wave branch in order, by cherry-pick, before anything is pushed. A single-issue wave has no threads and nothing to fold.
